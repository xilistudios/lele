...
export function Sidebar({ collapsed, mobileOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        <div
          className={`border-b border-[#2e2e2e] px-4 py-3 ${hideText ? 'opacity-0' : ''}`}
        >
        ...
        <div
          className={`border-b border-[#2e2e2e] px-3 py-3 ${hideText ? 'hidden' : ''}`}
        >
          <div className="overflow-hidden transition-all duration-300 ease-in-out max-h-8 opacity-100">
        ...
        <div
          className={`flex items-center justify-between border-t border-[#2e2e2e] px-4 py-3 ${hideText ? 'hidden' : ''}`}
        >
          <Popover
            trigger={
