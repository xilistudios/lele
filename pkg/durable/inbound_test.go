...
// rawEnqueue writes a spool row directly, bypassing EnqueueAndPublish, so tests
...
// payloadFor is the JSON shape EnqueueAndPublish writes for msg.
...
// ──────────────────────────────────────────────────────────────────────────────
// Durability off (nil repo): degrade to a plain publish
// ──────────────────────────────────────────────────────────────────────────────

func TestNilRepoDegradesToPlainPublish(t *testing.T) {
	b := newFakeBus(8)
	d := NewInbound(nil, b.publish)

	got := d.EnqueueAndPublish(msg("hello", "m-1"))

	// The message still flows, and it still gets a dedupe key: the ledger is
	// useful even without the spool if the flag is turned on later.
	if got.SpoolID != 0 {
		t.Errorf("SpoolID = %d, want 0 with no repo", got.SpoolID)
	}
	if got.DedupeID != "m-1" {
		t.Errorf("DedupeID = %q, want %q", got.DedupeID, "m-1")
	}
...
func TestNilPublisherNeverPanics(t *testing.T) {
	...
	// The row is the only copy left, so it must survive for the pump.
...
func TestEnqueueAndPublishWritesRowAndFillsIDs(t *testing.T) {
	f := openSpool(t)
	repo := f.repo
	b := newFakeBus(8)
...
func TestEnqueueAndPublishIsIdempotent(t *testing.T) {
	f := openSpool(t)
	repo := f.repo
	b := newFakeBus(8)
	d := NewInbound(repo, b.publish)

	first := d.EnqueueAndPublish(msg("hello", "m-1"))
	second := d.EnqueueAndPublish(first)
	...
	if b.callCount() != 2 {
		t.Errorf("publish called %d times, want 2", b.callCount())
	}
}
...
func TestEnqueueSynthesizesDedupeIDWithoutMessageID(t *testing.T) {
	...
	got := d.EnqueueAndPublish(msg("hello", ""))
...
func TestDedupeIDsAreUniqueAcrossMessages(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		id := d.EnqueueAndPublish(msg("x", "")).DedupeID
		if seen[id] {
			t.Fatalf("duplicate synthesized DedupeID %q", id)
		}
		seen[id] = true
	}
}

func TestEnqueueFailureStillPublishes(t *testing.T) {
	f := openSpool(t)
	repo := f.repo
	b := newFakeBus(8)
	d := NewInbound(repo, b.publish)

	// Close the database to make the durable write fail. A live user message
	// must still reach the bus: availability wins over durability.
	f.closeDB(t)
...
func TestRestartReplaysExactlyOnce(t *testing.T) {
	...
	m1 := first.EnqueueAndPublish(msg("first", "m-1"))
	first.Finish(m1)

	// A second message arrives but the process dies before it is answered.
	m2 := first.EnqueueAndPublish(msg("second", "m-2"))
	if m2.SpoolID == 0 {
