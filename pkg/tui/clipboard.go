...
// xclip, xsel, wl-copy, and pbcopy in that order. Reports whether anything
// was copied so the caller can show feedback.
func (m *Model) copyLastAssistantMessage() bool {
	if m.currentKey == "" {
		return false
	}
...
// maxOSC52Bytes caps the OSC 52 payload. Larger copies rely on the platform
// clipboard fallback — some terminals reject or truncate huge OSC sequences.
const maxOSC52Bytes = 7500
