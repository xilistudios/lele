...
#### TUI
- Queued-message preview band — when the composer is parked behind a busy agent, the pending backlog now shows *what* is about to be sent (FIFO head) right above the input, dimmed and truncated to the pane width. The band costs a line only while the queue is non-empty: the viewport budget measures the rendered string, so an idle queue keeps the full transcript height. Multi-line payloads are collapsed to a single sanitized line (control sequences stripped, tabs expanded), since the band is budgeted exactly one row.
