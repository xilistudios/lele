...
<h1>Lele: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 Lele は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 Lele リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！
...
### 🐜 革新的な省フットプリントデプロイ
Lele はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/xilistudios/lele/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに Lele を実行できます。
...
> [!TIP]
> `~/.lele/config.json` に API キーを設定してください。
...
Telegram、Discord、QQ、DingTalk、LINE で Lele と会話できます
...
> **Docker Compose**: `lele-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。

</details>

## ⚙️ 設定

設定ファイル: `~/.lele/config.json`

### ワークスペース構成

Lele は設定されたワークスペース（デフォルト: `~/.lele/workspace`）にデータを保存します：
...
Lele はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.lele/workspace` | エージェントの作業ディレクトリ |
...
Lele は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `LELE_HEARTBEAT_ENABLED=false` で無効化
- `LELE_HEARTBEAT_INTERVAL=60` で間隔変更
...
| コマンド | 説明 |
|---------|------|
| `lele onboard` | 設定＆ワークスペースの初期化 |
| `lele agent -m "..."` | エージェントとチャット |
| `lele agent` | インタラクティブチャットモード |
| `lele gateway` | ゲートウェイを起動 |
| `lele status` | ステータスを表示 |
...
別のインスタンスが実行中の場合に発生します。`lele gateway` が 1 つだけ実行されていることを確認してください。
